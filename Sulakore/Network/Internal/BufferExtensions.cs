...
internal static class BufferExtensions
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static ReadOnlyMemory<byte> ToMemory(in this ReadOnlySequence<byte> buffer)
    {
        if (buffer.IsSingleSegment)
        {
            return buffer.First;
    ...
    internal static void WriteAscii<T>(ref this BufferWriter<T> buffer, string data)
    ...
    [MethodImpl(MethodImplOptions.NoInlining)]
    private static void WriteAsciiMultiWrite<T>(ref this BufferWriter<T> buffer, string data)
        where T : IBufferWriter<byte>
    {
