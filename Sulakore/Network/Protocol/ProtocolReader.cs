...
internal sealed class ProtocolReader : IAsyncDisposable
{
    ...
    public ProtocolReader(PipeReader reader) => _reader = reader;
    ...
    {
        if (_disposed) throw new ObjectDisposedException(nameof(ProtocolReader));
        if (_hasMessage) throw new InvalidOperationException($"{nameof(Advance)} must be called before calling {nameof(ReadAsync)}");
    ...
    private ValueTask<ProtocolReadResult<TReadMessage>> DoAsyncRead<TReadMessage>(IMessageReader<TReadMessage> reader, CancellationToken cancellationToken)
    {
        while (true)
        {
            ...
            else return ContinueDoAsyncRead(readTask, reader, cancellationToken);

            (bool shouldContinue, bool hasMessage) = TrySetMessage(result, reader, out var protocolReadResult);
            if (hasMessage)
            {
